...
| `CLAUDE_MEM_OPENROUTER_BASE_URL` | string | — | Custom endpoint URL (optional, for proxies/self-hosted) |
...
You can override the default OpenRouter API endpoint to use proxy servers, self-hosted gateways, or custom implementations.

### Use Cases

- **Proxy Servers**: Route API requests through a corporate proxy
- **Self-Hosted Gateways**: Use custom API implementations
- **Load Balancers**: Distribute requests across multiple endpoints
...
```json
{
  "CLAUDE_MEM_OPENROUTER_BASE_URL": "https://my-gateway.example.com/api/v1/chat/completions"
}
```
...
```bash
export OPENROUTER_BASE_URL="https://my-gateway.example.com/api/v1/chat/completions"
```

### Important Notes

- Leave empty to use the default endpoint (`https://openrouter.ai/api/v1/chat/completions`)
- The URL must be the **full chat completions endpoint** - nothing will be appended
- Example: `https://proxy.com/api/v1/chat/completions` (complete URL)

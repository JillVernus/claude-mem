...
| `CLAUDE_MEM_GEMINI_RATE_LIMITING_ENABLED` | `true`, `false` | `true` | Enable rate limiting for free tier (disable if billing enabled on Google Cloud) |
| `CLAUDE_MEM_GEMINI_BASE_URL` | string | — | Custom models base URL (optional, for proxies/self-hosted) |
...
You can override the default Gemini API endpoint to use proxy servers, self-hosted gateways, or regional endpoints.
...
```json
{
  "CLAUDE_MEM_GEMINI_BASE_URL": "https://my-proxy.example.com/v1beta/models"
}
```
...
```bash
export GEMINI_BASE_URL="https://my-proxy.example.com/v1beta/models"
```

### Important Notes

- Leave empty to use the default endpoint (`https://generativelanguage.googleapis.com/v1beta/models`)
- The URL should be the **models base** - the model name will be appended automatically
- Example: If you set `https://proxy.com/v1beta/models`, requests will go to `https://proxy.com/v1beta/models/gemini-2.5-flash:generateContent`
- Custom URLs must use `http://` or `https://` protocol
- Settings file takes precedence over environment variable

<Warning>
**Security**: Only use trusted endpoints. Custom endpoints can intercept your API keys. Avoid using `http://` (unencrypted) in production.
</Warning>
